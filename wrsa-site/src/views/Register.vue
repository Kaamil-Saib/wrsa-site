<template>
  <div class="auth-wrapper">
    <div class="auth-container">
      ...
      <p class="auth-sub">Join the crew. Gear up for the wild.</p>

      <div class="input-group">
        <div class="name-row">
          <input type="text" placeholder="First Name" v-model.trim="firstName">
          <input type="text" placeholder="Surname" v-model.trim="surname">
        </div>
        <input type="email" placeholder="Email Address" v-model.trim="email" @keyup.enter="register">
...
const firstName = ref(""); const surname = ref(""); const email = ref(""); const password = ref("");
const router = useRouter(); const db = getFirestore();

const createUserDoc = async (user, first, last) => {
  await setDoc(doc(db, "uid", user.uid), { firstName: first, surname: last, email: user.email, role: "user", createdAt: new Date() });
};

const register = async () => {
  if (!firstName.value || !surname.value || !email.value || !password.value) { emit('notify', "Please fill in all fields", "error"); return; }
  try {
    ...
    emit('notify', `Welcome, ${firstName.value}!`, "success"); router.push("/");
  } catch (error) {
...
const signInWithGoogle = async () => {
  try {
    ...
    await setDoc(doc(db, "uid", res.user.uid), { firstName: parts[0] || "", surname: parts.slice(1).join(" ") || "", email: res.user.email, role: "user" }, { merge: true });
